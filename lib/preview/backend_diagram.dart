...
class BackendDiagramPreview extends StatelessWidget {
  ...
  const BackendDiagramPreview({Key? key, required this.backendJson})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    if (backendJson.isEmpty) {
      return const Center(
        child: Text("Waiting for Backend architecture generation..."),
      );
    }
  ...
  Widget _buildNode(dynamic nodeData) {
    ...
    final type =
        nodeData['type'] ??
        'service'; // 'ui' (blue) or 'service' (black outline)
